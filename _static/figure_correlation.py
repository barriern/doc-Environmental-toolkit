...
# load the dataset
fin = Dataset("_static/combined_gridded_index_data.nc", "r")
...
for lagtest in [-1, 0, 1]:
    ...
    ax = plt.subplot(3, 2, cpt, projection=ccrs.PlateCarree())
    if cpt==1:
        plt.title('NAO')
    cs1 = ax.contourf(lon, lat, corrpy[0, :, :, ilag], levels=levels)
    ax.coastlines()
    cpt += 1
    ...
    if cpt==2:
        plt.title("AMO")
    cs = ax.contourf(lon, lat, corrpy[1, :, :, ilag], levels=levels)
    ax.coastlines()
    cpt +=1 
    plt.text(10, 70, "lag %d"%lagtest, bbox=bbox)
...
plt.savefig("_static/figure_correlation.png", bbox_inches='tight')
