...
filein = Dataset("_static/ts_sst.nc", "r")
data = filein.variables["sst"][:, 0, 0]
time = filein.variables["time"]
units = time.units
time = time[:]
date = cftime.num2date(time, units)
filein.close()
...
datem = cftime.num2date(timem, units)
...
plt.savefig("_static/figure_climato.png")
...
plt.ylabel("SST clim")
plt.legend(loc=0)
plt.savefig("_static/figure_climato_2.png")
